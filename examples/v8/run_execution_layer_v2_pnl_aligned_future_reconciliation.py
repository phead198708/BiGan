...
def main() -> None:
    ...
    parser.add_argument("--settled-evaluation-rows-jsonl", required=True, type=Path)
    ...
    report, rows = evaluate_pnl_aligned_future_accepted_bets(
        ...
        settled_evaluation_rows=_load_jsonl(args.settled_evaluation_rows_jsonl),
    )
    ...
    manifest = {
        "schema_version": (
            "bigan-v8-execution-layer-v2-pnl-aligned-future-evaluation-manifest-v1"
        ),
        "run_id": args.run_id,
        "shadow_manifest": _descriptor(args.shadow_manifest),
        "settled_evaluation_rows": _descriptor(args.settled_evaluation_rows_jsonl),
