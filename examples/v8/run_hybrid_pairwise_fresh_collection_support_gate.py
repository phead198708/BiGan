#!/usr/bin/env python3
"""Run #185 bounded support and outcome-blind 45/60 role assignment."""
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--collection-launch-plan", required=True)
    parser.add_argument("--collection-launch-plan-sha256", required=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = run_hybrid_fresh_collection_support_gate(
        HybridFreshCollectionSupportGateConfig(
            run_id=args.run_id,
            output_dir=args.output_dir,
            collection_launch_plan_path=args.collection_launch_plan,
            expected_collection_launch_plan_sha256=(
                args.collection_launch_plan_sha256
            ),
        ...
        json.dumps(
            {
                ...
                "role_assignment_manifest_path": (
                    ...
                    else str(role_result["manifest_path"])
                ),
                "report_path": str(result["report_path"]),
                "report_sha256": result["report_sha256"],
                "manifest_path": str(result["manifest_path"]),
                "manifest_sha256": result["manifest_sha256"],
                "labels_or_outcomes_opened_for_support": False,
                "paper_only": True,
