"""Frozen #173 expected-mean LCB model and one-shot confirmatory gate."""
...
SCHEMA_PREFIX = "bigan-v8-execution-layer-v2-execution-compatible-mean-lcb"
...
FAMILY_MODEL_FILENAMES = {
    "HOLD_TO_SETTLEMENT": "execution_compatible_mean_lcb_hts_model.xgb.json",
    "SELL_BEFORE_CLOSE": "execution_compatible_mean_lcb_sbc_model.xgb.json",
}
...
class ExecutionCompatibleMeanLCBFitConfig:
...
def fit_execution_compatible_mean_lcb(
    config: ExecutionCompatibleMeanLCBFitConfig,
) -> dict[str, Any]:
    """Fit on 60 markets, calibrate mean LCB on 30, evaluate once on 30."""
    ...
    for role, rows in action_rows_by_role.items():
        path = run_dir / f"execution_compatible_mean_lcb_{role}_action_rows.jsonl"
    ...
    oof_path = run_dir / "execution_compatible_mean_lcb_train_oof_predictions.jsonl"
    ...
    calibration_report_path = run_dir / "expected_mean_lcb_calibration_report.json"
    _write_json(calibration_report_path, calibration_report)
    _write_text(
        run_dir / "expected_mean_lcb_calibration_report.md",
    ...
    development_fit_freeze = {
        ...
        "expected_mean_lcb_calibration_artifact": _descriptor(lcb_path),
        "expected_mean_lcb_calibration_report": _descriptor(calibration_report_path),
    ...
    development_fit_freeze_path = (
        run_dir / "execution_compatible_mean_lcb_development_fit_freeze_manifest.json"
    )
    ...
    confirmatory_action_path = (
        run_dir / "execution_compatible_mean_lcb_confirmatory_validation_action_rows.jsonl"
    )
    _write_jsonl(
        confirmatory_action_path,
    ...
    split_manifest_path = run_dir / "execution_compatible_mean_lcb_split_manifest.json"
    ...
    confirmatory_predictions = _apply_expected_mean_lcb_scores(
        confirmatory_predictions,
        lcb_artifact=lcb_artifact,
    )
    prediction_path = run_dir / "execution_compatible_mean_lcb_confirmatory_predictions.jsonl"
    ...
    candidate_replay = _run_policy_replay(
        confirmatory_predictions,
        score_field="expected_mean_lcb_net_return",
    ...
    candidate_path = run_dir / "execution_compatible_mean_lcb_confirmatory_replay.jsonl"
    baseline_path = run_dir / "execution_compatible_mean_lcb_confirmatory_baseline_replay.jsonl"
    ...
    freeze_manifest["research_candidate_hash"] = canonical_json_sha256(
        {
            ...
            "lcb": freeze_manifest["expected_mean_lcb_calibration_artifact"][
                "sha256"
            ],
        }
    )
...
def _validate_role_rows(rows: list[dict[str, Any]]) -> None:
    ...
    if any(
        row.get("execution_compatibility_validated_before_label_access") is not True
        for row in rows
    ):
...
def _cross_fit_training_predictions(
    ...
    *,
    ...
    if len(ordered_markets) != 60 or fold_count != 5:
        raise ValueError("cross-fit requires exactly 60 markets and five folds")
    markets_per_fold = len(ordered_markets) // fold_count
    fold_market_groups = [
        ordered_markets[
            index * markets_per_fold : (index + 1) * markets_per_fold
        ]
...
def _expected_mean_lcb_artifact(
    ...
    *,
    ...
    lcb_protocol = dict(protocol["expected_mean_lcb_protocol"])
    ...
    minimum_group_markets = int(
        lcb_protocol["minimum_calibration_unique_markets_per_group"]
    )
    minimum_family_markets = int(
        lcb_protocol["minimum_calibration_unique_markets_per_family"]
    )
    ...
    for family_index, family in enumerate(TRADE_FAMILIES):
        ...
        if not oof_scores:
            raise ValueError(f"train OOF score coverage is missing for {family}")
        boundaries = [
        ...
        family_stats = _market_grouped_mean_residual_ci(
            ...
            seed=bootstrap_seed + family_index * 10_000,
        )
        if family_stats["unique_market_count"] < minimum_family_markets:
            raise ValueError(f"insufficient calibration market support for {family}")
        families[family] = {
            ...
            "mean_residual": family_stats["mean_residual"],
        ...
        for bucket_index, bucket_name in enumerate(("low", "middle", "high")):
            bucket_rows = [
                ...
                for row in rows
                if _score_bucket(
                    float(row["raw_family_expected_net_return"]), boundaries
                )
                == bucket_name
            ]
            group_stats = _market_grouped_mean_residual_ci(
                ...
                confidence_level=confidence_level,
                bootstrap_resample_count=bootstrap_resample_count,
                seed=bootstrap_seed + family_index * 10_000 + bucket_index + 1,
            )
            support_passed = (
                group_stats["unique_market_count"] >= minimum_group_markets
            )
            if support_passed:
                weight = group_stats["unique_market_count"] / (
                    group_stats["unique_market_count"] + shrinkage_prior_markets
                )
                penalty = weight * float(
                    group_stats["mean_residual_upper_confidence_bound"]
                ) + (1.0 - weight) * float(
                    family_stats["mean_residual_upper_confidence_bound"]
                )
                penalty_source = "shrunken_group_and_family_mean_residual_ci"
            else:
                weight = 0.0
                penalty = float(
                    family_stats["mean_residual_upper_confidence_bound"]
                )
                penalty_source = "family_level_mean_residual_ci_fallback"
            if not math.isfinite(penalty):
                raise ValueError("expected-mean residual penalty must be finite")
            key = f"{family}|{bucket_name}"
            calibration_groups[key] = {
                "action_family": family,
                "score_bucket": bucket_name,
                "calibration_row_count": len(bucket_rows),
                "calibration_unique_market_count": group_stats[
                    "unique_market_count"
                ],
                ...
                "family_mean_residual_upper_confidence_bound": family_stats[
                    "mean_residual_upper_confidence_bound"
                ],
    ...
    artifact = {
        ...
        "decision_score_formula": (
            ...
            "expected_mean_residual_upper_confidence_bound"
        ),
        "confidence_level": confidence_level,
        ...
        "insufficient_group_support_fallback": lcb_protocol[
            "insufficient_group_support_fallback"
        ],
...
def _market_grouped_mean_residual_ci(
    ...
    *,
    ...
    upper = float(
        np.quantile(bootstrap_means, confidence_level, method="higher")
    )
...
def _apply_expected_mean_lcb_scores(
    ...
    *,
    ...
    for row in predictions:
        ...
        if family == "NO_TRADE":
            bucket = "none"
            penalty = 0.0
            penalty_source = "no_trade_zero_score"
        else:
            boundaries = list(
                lcb_artifact["families"][family][
                    "train_oof_score_tertile_boundaries"
                ]
            )
            bucket = _score_bucket(raw_score, boundaries)
            group = lcb_artifact["calibration_groups"][f"{family}|{bucket}"]
            penalty = float(
                group["expected_mean_residual_upper_confidence_bound"]
            )
            penalty_source = str(group["penalty_source"])
        updated = {
            **row,
            "expected_mean_lcb_score_bucket": bucket,
            "expected_mean_residual_upper_confidence_bound": penalty,
            "expected_mean_lcb_penalty_source": penalty_source,
            "expected_mean_lcb_net_return": raw_score - penalty,
...
def _training_markdown(report: dict[str, Any]) -> str:
    return "\n".join(
        [
            "# #173 Cross-Fitted Family LCB Training",
            "",
...
def _calibration_markdown(report: dict[str, Any]) -> str:
    return "\n".join(
        [
            "# #173 Conformal Family LCB Calibration",
            "",
...
def _validation_markdown(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# #173 Untouched Confirmatory Validation",
            "",
