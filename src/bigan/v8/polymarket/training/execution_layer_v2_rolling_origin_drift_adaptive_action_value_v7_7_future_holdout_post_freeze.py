"""Single-use read-only settlement and PnL gate for issue #241."""
...
class V77FutureSettlementConfig:
...
@dataclass(frozen=True, slots=True)
class V77FutureEvaluationConfig:
...
def build_v7_7_future_settled_index(
    config: V77FutureSettlementConfig,
    *,
    ...
    """Settle copied exact-120 rounds after an exclusive target-access claim."""
    ...
    if config.target_access_started_ts <= max(freeze_created_ts, max_market_end_ts):
        raise ValueError("#241 target access attempted before freeze or market close")
    ...
    claim_path = freeze_path.parent / "v7_7_future_single_use_target_access_claim.json"
    _write_single_use_claim(claim_path, claim)
    marker_path = run_dir / "v7_7_future_settlement_start_marker.json"
    ...
    if finalized_ts < config.target_access_started_ts:
        raise ValueError("#241 settlement finalized before target access start")
    index_path = run_dir / "v7_7_future_settled_corpus_index.json"
    ...
    report_path = run_dir / "v7_7_future_settlement_report.json"
    report_md_path = run_dir / "v7_7_future_settlement_report.md"
    ...
    manifest_path = run_dir / "v7_7_future_settlement_manifest.json"
...
def evaluate_v7_7_future_pnl_gate(
    config: V77FutureEvaluationConfig,
) -> dict[str, Any]:
    """Consume complete official targets once and compare frozen v7.7 with v6.7."""
    ...
    _verify_pin(settled_path, config.expected_settled_index_sha256, "#241 settled index")
    _verify_pin(
        runtime_path,
        config.expected_runtime_policy_profile_sha256,
        "#241 runtime policy profile",
    )
    plan_descriptor = _verified_descriptor(freeze["plan"], "#241 frozen plan")
    if plan_descriptor["sha256"] != FROZEN_PLAN_SHA256:
        raise ValueError("#241 freeze references a different plan")
    ...
    if (
        ...
        raise ValueError("#241 runtime policy profile pin drifted")
    ...
    claim["claim_id"] = canonical_json_sha256(claim)
    claim_path = freeze_path.parent / "v7_7_future_single_use_pnl_gate_claim.json"
    _write_single_use_claim(claim_path, claim)
    ...
    candidate_targets = _runtime_targets_for_decisions(
        ...
        run_id=config.run_id,
        role="future_unseen_holdout_candidate",
    )
    ...
    gate = build_v7_7_future_pnl_noninferiority_gate(
        candidate_targets,
    ...
    candidate_path = run_dir / "v7_7_future_candidate_runtime_targets.jsonl"
    baseline_path = run_dir / "v7_7_future_v6_7_runtime_targets.jsonl"
    ...
    report = {
        **gate,
        ...
        "target_access_claim": _descriptor(
            Path(_verified_descriptor(settled["target_access_claim"], "#241 target claim")["path"])
        ),
    ...
    report_path = run_dir / "v7_7_future_pnl_noninferiority_gate_report.json"
    report_md_path = run_dir / "v7_7_future_pnl_noninferiority_gate_report.md"
    _write_json(report_path, report)
    ...
    manifest["manifest_id"] = canonical_json_sha256(manifest)
    manifest_path = run_dir / "v7_7_future_pnl_noninferiority_gate_manifest.json"
    _write_json(manifest_path, manifest)
...
def _validated_freeze(
    ...
    _verify_pin(path, expected_sha256, "#241 target-free freeze")
    ...
    if (
        ...
        or freeze.get("threshold_or_model_tuning_performed") is not False
    ):
        raise ValueError("#241 target-free freeze is not target-access eligible")
    for field, expected in _safety_fields().items():
        if freeze.get(field) != expected:
            raise ValueError(f"#241 target-free freeze safety mismatch: {field}")
    selected = _load_jsonl(
        Path(_verified_descriptor(freeze["selected_rows"], "#241 selected rows")["path"])
    )
    candidate = _load_jsonl(
        Path(_verified_descriptor(freeze["v7_7_runtime"], "#241 v7.7 decisions")["path"])
    )
    baseline = _load_jsonl(
        Path(_verified_descriptor(freeze["v6_7_runtime"], "#241 v6.7 decisions")["path"])
    )
    ...
    if (
        ...
        or len(candidate) < 40
        or any(str(row.get("market_id") or "") not in set(market_ids) for row in candidate + baseline)
    ):
        raise ValueError("#241 frozen market or accepted-decision support invalid")
...
def _validate_settled_index(
    ...
    *,
    ...
    freeze_descriptor = _verified_descriptor(
        index["target_free_freeze_manifest"], "#241 settled freeze"
    )
    if (
        ...
        raise ValueError("#241 settled index is not evaluation eligible")
    _verified_descriptor(index["target_access_claim"], "#241 target access claim")
    for field, expected in _safety_fields().items():
        if index.get(field) != expected:
            raise ValueError(f"#241 settled index safety mismatch: {field}")
    for entry in entries:
        if (
            entry.get("official_read_only_resolution") is not True
            or entry.get("source_outcome_blind_round_mutated") is not False
        ):
            raise ValueError("#241 settled entry violates quarantine contract")
        for name in ("feature_rows", "label_rows", "resolution_events"):
            _verified_descriptor(entry[name], f"#241 settled {name}")
    return entries
...
def _write_single_use_claim(path: Path, claim: dict[str, Any]) -> None:
    try:
        ...
        raise ValueError("#241 frozen future targets have already been consumed") from error
...
def _validate_common_config(
    *, run_id: str, implementation_commit: str, stage_started_ts: int
) -> None:
    if not run_id.strip():
...
def _settlement_markdown(report: dict[str, Any]) -> str:
    return "\n".join(
        [
            "# v7.7 Future Read-Only Settlement",
            "",
...
def _evaluation_markdown(report: dict[str, Any]) -> str:
    return "\n".join(
        [
            "# v7.7 Future PnL Non-Inferiority Gate",
            "",
