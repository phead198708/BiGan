...
CANONICAL_FEATURE_COMPARISON_REPORT_FILENAME = "canonical_feature_payload_comparison_report.json"
...
def _finalize_selected_rounds(
    ...
    *,
    ...
    evaluation_only_frozen_features_by_market: (dict[str, list[dict[str, Any]]] | None) = None,
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(
                ...
                require_complete_raw_feature_lineage=(require_complete_raw_feature_lineage),
            ): selected
...
def _copy_and_finalize_selected_round(
    ...
    *,
    ...
    if (
        (report.get("finalization_status") != "exported" and not evaluation_only_fallback)
        ...
        canonical_report_path = copied_run_dir / CANONICAL_FEATURE_COMPARISON_REPORT_FILENAME
        if canonical_report_path.is_file():
            failure["failure"]["canonical_feature_payload_comparison_report"] = _descriptor(
                canonical_report_path
            )
        return failure
    corpus_dir = (
        evaluation_only_corpus_dir if evaluation_only_fallback else result.corpus_dir.resolve()
    )
    ...
    if evaluation_only_frozen_feature_rows is not None:
            ...
            if canonical_report["canonical_comparison_passed"] is not True:
                return {
                    ...
                    "failure": {
                        "market_id": market_id,
                        "run_id": source_run_id,
                        "reason_codes": ["settled_feature_canonical_comparison_failed"],
        ...
        if not canonical_report_path.is_file():
            raise ValueError("canonical feature comparison report was not materialized")
    index_entry = {
        "market_id": market_id,
        "run_id": source_run_id,
        ...
        "canonical_feature_payload_comparison_report": (
            _descriptor(canonical_report_path) if canonical_report_path is not None else None
        ),
...
def _evaluation_only_settled_corpus_if_safe(
    *,
    ...
    if sum(1 for line in label_rows_path.read_text(encoding="utf-8").splitlines() if line) <= 0:
        reasons.append("evaluation_only_label_rows_empty")
    if (
        sum(1 for line in resolution_events_path.read_text(encoding="utf-8").splitlines() if line)
        != 1
    ):
...
def _write_canonical_feature_comparison_report(
    *,
    ...
    corpus_manifest = _load_json(corpus_manifest_path)
    settled_features = _load_jsonl(feature_rows_path)
    normalized = dict(corpus_manifest.get("normalized_artifact_hashes") or {})
    ...
    source_raw = {
        ...
        for name, descriptor in dict(source_evidence.get("raw_artifacts") or {}).items()
    }
    corpus_raw = {
        str(name): str(digest or "")
        for name, digest in dict(corpus_manifest.get("raw_artifact_hashes") or {}).items()
    }
    expected_raw_names = set(ALLOWED_RAW_FEATURE_FILES)
    raw_lineage_matches = all(corpus_raw.get(name) == digest for name, digest in source_raw.items())
    if require_complete_raw_feature_lineage:
        raw_lineage_matches = expected_raw_names.issubset(source_raw) and all(
            corpus_raw.get(name) == source_raw[name] for name in expected_raw_names
        )
    lineage_checks = {
        **{
        ...
        "finalization_manifest_corpus_path_matches": (
            str(finalization_manifest.get("phase2_corpus_dir") or "") == str(corpus_dir.resolve())
        ),
        "finalization_manifest_corpus_hash_matches": (
            str(finalization_manifest.get("phase2_corpus_manifest_sha256") or "")
            == _sha256_file(corpus_manifest_path)
        ),
        "normalized_feature_rows_hash_matches": (
            normalized.get("feature_rows") == _sha256_file(feature_rows_path)
        ),
        ...
        "normalized_resolution_events_hash_matches": (
            normalized.get("resolution_events") == _sha256_file(resolution_events_path)
        ),
        "source_raw_artifact_hashes_match_settled_corpus": (raw_lineage_matches),
    }
    ...
    comparison = build_canonical_payload_comparison_report(
        ...
        source_lineage_evidence={
            ...
            "settled_resolution_events": _descriptor(resolution_events_path),
...
def _verify_selected_source_unchanged(
    ...
    *,
    ...
    required_raw_artifacts_complete = set(ALLOWED_RAW_FEATURE_FILES).issubset(
    ...
    return {
        ...
        "checks": {
            ...
            "required_raw_feature_artifacts_complete": (
                required_raw_artifacts_complete if require_complete_raw_feature_lineage else True
            ),
        },
        "source_pending_capture_manifest": capture_manifest_descriptor,
...
def reconcile_conformal_v5_future_settlement(
    ...
    candidate_replay_descriptor = _verified_descriptor(
        freeze_manifest["candidate_outcome_blind_guard_replay"],
        "candidate guard replay",
    )
