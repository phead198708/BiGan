"""Precollection freeze for the #173 execution-compatible expected-mean LCB."""
...
CANDIDATE_NAME = "market_grouped_cross_fitted_expected_mean_lcb_v1"
ROLE_MARKET_COUNTS = {
...
class ExecutionCompatibleMeanLCBPrecollectionFreezeConfig:
...
@dataclass(frozen=True, slots=True)
class ExecutionCompatibleMeanLCBRoleAssignmentConfig:
...
def validate_execution_compatible_mean_lcb_protocol(protocol: dict[str, Any]) -> None:
    ...
    expected_mean = dict(protocol.get("expected_mean_lcb_protocol") or {})
    ...
    checks = {
        ...
        == 1.0
        and float(
            collector.get("maximum_selected_side_book_staleness_ms") or 0.0
        )
        == 2_000.0
        and float(
            collector.get("maximum_opposite_side_book_staleness_ms") or 0.0
        )
        == 2_000.0
        and collector.get("complete_up_down_executable_book_required") is True
        and collector.get("execution_compatibility_validated_before_label_access")
        is True
        ...
        "calibration_only_expected_mean_lcb": expected_mean.get("source_split")
        == "development_calibration_only"
        and expected_mean.get("estimand")
        == "conditional_expected_cost_aware_net_return"
        and expected_mean.get("grouping")
        == "action_family_x_train_oof_score_tertile"
        and expected_mean.get("score_bucket_boundaries_source")
        == "development_train_oof_predictions_only"
        and expected_mean.get("bootstrap_unit") == "market_id"
        and int(expected_mean.get("bootstrap_resample_count") or 0) >= 1_000
        and isinstance(expected_mean.get("bootstrap_seed"), int)
        and int(
            expected_mean.get("minimum_calibration_unique_markets_per_group") or 0
        )
        == 10
        and expected_mean.get("affine_calibration_enabled") is False
        and expected_mean.get("individual_outcome_quantile_subtraction_enabled")
        is False,
    ...
    if failed:
        raise ValueError(
            "invalid execution-compatible mean LCB protocol: " + ", ".join(failed)
        )


def validate_execution_compatible_mean_lcb_feature_contract(
    ...
    *,
    ...
    if failed:
        raise ValueError(
            "invalid execution-compatible mean LCB feature contract: "
            + ", ".join(failed)
        )


def freeze_execution_compatible_mean_lcb_precollection(
    config: ExecutionCompatibleMeanLCBPrecollectionFreezeConfig,
) -> dict[str, Any]:
    ...
    batch_id_prefix = f"issue173-{config.run_id}"
    manifest = {
        ...
        "expected_mean_lcb_protocol": protocol["expected_mean_lcb_protocol"],
        "expected_mean_lcb_protocol_sha256": canonical_json_sha256(
            protocol["expected_mean_lcb_protocol"]
        ),
...
def assign_execution_compatible_mean_lcb_roles(
    config: ExecutionCompatibleMeanLCBRoleAssignmentConfig,
) -> dict[str, Any]:
    """Assign the earliest 60/30/30 execution-compatible markets outcome-blind."""
    ...
    for capture in batch_rows:
        ...
        if not corpus_reasons:
            ...
            corpus_reasons.extend(
                execution_compatibility_audit["blocking_reason_codes"]
            )
        if corpus_reasons:
    ...
    selected_path = run_dir / "execution_compatible_mean_lcb_role_assignment_rows.jsonl"
    excluded_path = run_dir / "execution_compatible_mean_lcb_role_assignment_excluded_rows.jsonl"
    ...
    compatibility_report_path = (
        run_dir / "execution_compatible_feature_coverage_report.json"
    )
    ...
    report_path = run_dir / "execution_compatible_mean_lcb_role_assignment_report.json"
    _write_json(report_path, report)
    markdown_path = run_dir / "execution_compatible_mean_lcb_role_assignment_report.md"
    ...
    manifest = {
        ...
        "execution_compatible_feature_coverage_report": _descriptor(
            compatibility_report_path
        ),
    ...
    *,
    ...
    for row in rows:
        ...
        missing_runtime = sorted(
            ...
            if not _finite_number(features.get(name))
        )
        if missing_runtime:
        ...
        for side in ("up", "down"):
            ...
            if (
                ...
                row_reasons.append(
                    f"execution_compatibility_{side}_executable_book_invalid"
                )
            ...
            if staleness is None or staleness < 0.0 or staleness > limit:
                row_reasons.append(
                    f"execution_compatibility_{side}_book_staleness_exceeded"
                )
            queue = _finite_float(
                features.get(f"{side}_queue_fill_probability_proxy")
            )
            if queue is None or not 0.0 <= queue <= 1.0:
                row_reasons.append(
                    f"execution_compatibility_{side}_queue_fill_invalid"
                )
...
def _role_for_rank(selection_rank: int) -> str:
    if 1 <= selection_rank <= 60:
        return "development_train"
    if selection_rank <= 90:
        return "development_calibration"
    if selection_rank <= 120:
        return "confirmatory_validation"
    raise ValueError("selection rank is outside the frozen 120-market role plan")
...
def _role_assignment_markdown(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# #173 Outcome-Blind Role Assignment",
            "",
...
def _execution_compatibility_markdown(report: dict[str, Any]) -> str:
    return "\n".join(
        [
            "# #173 Execution-Compatible Feature Coverage",
            "",
            f"- selected markets: `{report['selected_market_count']}`",
