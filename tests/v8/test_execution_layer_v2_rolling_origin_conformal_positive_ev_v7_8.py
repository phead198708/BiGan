...
PROFILE_PATH = (
    ...
    "execution_layer_v2_rolling_origin_conformal_positive_ev_v7_8_profile.json"
)
...
def _fit(
    ...
    return v78.fit_rolling_origin_conformal_positive_ev_v7_8(
...
def test_profile_and_finite_sample_quantile_are_frozen() -> None:
    profile = _profile()
    v78.validate_rolling_origin_conformal_positive_ev_v7_8_profile(profile)
    assert profile["conformal_contract"]["one_sided_quantile"] == 0.9
    assert (
        profile["historical_prequential_hard_gate"][
            "minimum_candidate_guard_accepted_unique_market_count"
        ]
        == 40
    )
    assert v78.finite_sample_upper_quantile(list(range(10)), 0.9) == 9.0

    changed = copy.deepcopy(profile)
    changed["conformal_contract"]["one_sided_quantile"] = 0.8
    with pytest.raises(ValueError, match="conformal"):
        v78.validate_rolling_origin_conformal_positive_ev_v7_8_profile(changed)


def test_strictly_prior_conformal_policy_passes_positive_historical_gate(
    ...
    state = model["final_conformal_state"]
    assert state["calibration_market_count"] == 214
    assert state["action_residual_count"] == 428
    assert state["paired_delta_residual_count"] == 214


def test_full_guard_blocks_without_faking_historical_support(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_target_free_scoring_uses_frozen_conformal_state_and_rejects_outcomes(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        v78.v77,
    ...
    model = {
        "schema_version": v78.MODEL_SCHEMA_VERSION,
        "historical_hard_gate_passed": True,
        "target_free_canary_collection_allowed": True,
        "final_weighted_model": {"available": True},
        "final_conformal_state": {
            "action_overprediction_q90": -0.1,
            "paired_delta_overprediction_q90": 0.0,
        },
    }
    market = _stream_markets()[0]
    decision = v78.score_rolling_origin_conformal_positive_ev_v7_8_market(
    ...
    blocked = v78.score_rolling_origin_conformal_positive_ev_v7_8_market(
        blocked_market, model_artifact=model
    )
    assert blocked["selected_action"] == "NO_TRADE"
    assert "v7_8_forbidden_outcome_field_in_inference_row" in blocked[
        "selection_reason_codes"
    ]


def test_issue241_result_paths_are_excluded_by_construction() -> None:
    annotations = v78.RollingOriginConformalPositiveEVV78Config.__annotations__
    assert not any("issue241" in name for name in annotations)
