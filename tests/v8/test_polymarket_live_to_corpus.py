...
def test_live_signal_converter_does_not_use_selected_side_as_label(
    ...
    resolutions = _read_jsonl(result.phase2_result.output_dir / "polymarket_resolution_events.jsonl")
    ...
    assert {row["resolved_outcome"] for row in labels} == {"UP"}
    up_hold_labels = [
        row for row in labels if row["action"] == "BUY_UP_HOLD_TO_SETTLEMENT"
    ]
...
def test_live_signal_converter_is_deterministic_for_identical_input(
    ...
    assert first.artifact_hashes["conversion_report"] == second.artifact_hashes[
        "conversion_report"
    ]
    assert first.artifact_hashes["raw_polymarket_markets.jsonl"] == second.artifact_hashes[
        "raw_polymarket_markets.jsonl"
    ]
    assert first.phase2_result is not None
    assert second.phase2_result is not None
    assert first.phase2_result.artifact_hashes["corpus_manifest"] == second.phase2_result.artifact_hashes[
        "corpus_manifest"
    ]
