...
def test_settlement_index_config_rejects_unbounded_or_unpinned_inputs(
    tmp_path: Path,
) -> None:
...
def test_exported_settlement_persists_canonical_feature_comparison(
    ...
    def fake_finalize(copied_run_dir: Path, **_: object) -> SimpleNamespace:
        ...
        manifest_path.write_text(
            json.dumps(
                {
                    "raw_artifact_hashes": {
                        name: descriptor["sha256"] for name, descriptor in raw_artifacts.items()
                    },
        ...
        finalization_path = copied_run_dir / "pending_round_finalization_manifest.json"
        finalization_path.write_text(
            json.dumps(
                {
                    "phase2_corpus_dir": str(corpus_dir.resolve()),
                    "phase2_corpus_manifest_sha256": _sha256(manifest_path),
                },
                sort_keys=True,
            )
            + "\n",
    ...
    container = result["index_entry"] if expected_ready else result["failure"]
    descriptor = container["canonical_feature_payload_comparison_report"]
...
def _canonical_feature_row(*, price: float) -> dict:
    return {
        ...
        "feature_provenance": {"execution_price": {"max_input_ts": 100}},
    }
