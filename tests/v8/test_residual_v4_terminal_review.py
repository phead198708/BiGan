...
from bigan.v8.polymarket.cost_aware_residual_v4 import DEFAULT_OUTPUT_DIR
from bigan.v8.polymarket.cost_aware_residual_v4_stacking import (
    DEFAULT_OUTPUT_DIR as DEFAULT_CHALLENGER_OUTPUT_DIR,
)
...
def _reports() -> tuple[dict, dict]:
    primary = _load_json(DEFAULT_OUTPUT_DIR / "residual_v4_oof_report.json")
    challenger = _load_json(
        DEFAULT_CHALLENGER_OUTPUT_DIR / "residual_v4_stacking_oof_report.json"
    )
...
def test_v4_terminal_review_blocks_all_unapproved_later_phases() -> None:
    ...
    assert review["best_candidate_required_prospective_market_count"] == 2488
...
def test_v4_terminal_review_rejects_gate_waiver_or_third_slot() -> None:
    primary, challenger = _reports()
...
def test_v4_terminal_review_rejects_source_artifact_omission() -> None:
    primary, challenger = _reports()
    descriptors = _descriptors()
    descriptors.pop("parent_v3_terminal_review")
