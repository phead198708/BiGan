...
CONTRACT_PATH = ROOT / "examples/v8/polymarket_configs/canonical_payload_contract.json"
CONTRACT_SHA_PATH = CONTRACT_PATH.with_suffix(".sha256")
FIXTURE_PATH = ROOT / "examples/v8/polymarket_configs/canonical_payload_cross_runtime_fixtures.json"
...
def test_comparison_report_derives_lineage_and_is_deterministic() -> None:
    ...
    assert first == second
    assert first["schema_version"] == CANONICAL_COMPARISON_REPORT_SCHEMA_VERSION
...
def test_comparison_report_fails_closed_when_any_lineage_check_fails() -> None:
    ...
    assert report["reason_codes"] == ["canonical_payload_source_lineage_not_approved"]
