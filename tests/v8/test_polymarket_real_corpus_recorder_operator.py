...
def test_live_orderbook_validation_does_not_require_uncollected_future_samples() -> None:
    ...
    market = discover_mock_market_rows(config)[0]
    decision_timestamps = sample_times_for_market(market, config)[2:4]
    rows = []
    for sample_index, decision_ts in enumerate(decision_timestamps):
        for outcome, token_id, mid in (
            ("UP", market["up_token_id"], 0.56 + sample_index * 0.01),
            ("DOWN", market["down_token_id"], 0.44 - sample_index * 0.01),
        ):
            rows.append(
                {
                    ...
                    "ts": decision_ts,
                    "available_at_ts": decision_ts,
                    "collection_end_ts": decision_timestamps[-1],
                    "bid_price": mid - 0.01,
                    "ask_price": mid + 0.01,
                    "mid_price": mid,
                    "bid_size": 100.0,
    ...
    assert reasons == []
    assert len(valid) == 4
    assert {row["ts"] for row in valid} == set(decision_timestamps)


def test_live_orderbook_validation_rejects_single_decision_timestamp() -> None:
    config = PolymarketRealCorpusRecorderConfig(
        run_id="single-live-book",
        output_dir="/tmp/single-live-book",
        market_families=("btc_updown_5m",),
        mock_public_data=False,
    )
    market = discover_mock_market_rows(config)[0]
